...
- `steps` は可変長の焼成工程配列（手順ラベル・温度℃・時間分）。カヌレのように「高温で予熱→温度を下げて焼く」といった複数段階の焼成を記録できる。旧フォーマットの単一 `bake_temp_c` / `bake_time_min` を持つノートは、読み込み時に自動的に1件の `steps` へ移行される。
...
| # | 機能 | 内容 |
|---|------|------|
| 1 | 試作記録の作成 | コマンドパレット/リボンアイコンからモーダルを開き、材料配合・焼成工程（複数段の温度/時間）・メモを入力してノート作成 |
